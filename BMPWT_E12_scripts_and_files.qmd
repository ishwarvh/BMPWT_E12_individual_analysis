...
nExp <- round(ncol(BMPWT_E12_seu_obj) * .14)  # expect ~8% doublets based on target cell recovery in this experiment (~10K cells), this changes for each sample.
...
# Threshold found:0.125
# 695 (5.7%) doublets called
...
saveRDS(scdbl_singlets, "BMPWT_E12_scdbl_singlets.RDS")
```
...
plot_ly_cols <-scCustomize_Palette(num_groups = 20, ggplot_default_colors = FALSE)
